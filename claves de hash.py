...
class Program: 
     def __init__(self):
          ...
          tk.Label(self.window,font=('Arial',20,'bold'),text="PasswordHash").grid(row=0,column=0,padx=25,pady=20)
          ...
          tk.Label(self.password_Frame,text="EnterPassword:").grid(pady=5,row=0,column=0)
          tk.Label(self.password_Frame,text="Confirmpassword:"+" "*6).grid(pady=5,row=1,column=0)
      ...
      self.output_Frame =tk.LabelFrame(self.window,text="Passwordhash",fg="blue",padx=25,pady=10) 
      ...
      tk.Button(self.window,text="CREATEHASH",command=self.init_task).grid(pady=20,row=4,column=0) 
     ...
     def check_passwords(self):
          if self.Entry1.get() != "" and self.Entry2.get() != "":
             if self.Entry1.get() == self.Entry2.get(): 
               ...
               messagebox.showwarning("ERROR","Passwordsdoesn't match")
               #self.Entry1.delete(0,tk.END)
               #self.Entry2.delete(0,tk.END)
          else:
               messagebox.showwarning("PASWORD NOT PROVIDED","Enter password for hashing and confirm") 
     
     def init_task(self):
          thearding.Thread(target=self.check_passwords).start()
