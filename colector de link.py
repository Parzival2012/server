...
class Collector:
    ...
    def __init__(self):
        ...
        tk.Button(self.root,text="SAVE URL AS:",width=30,bg="green",command=self.enter_name).place(x=5,y=70)#79

        tk.Button(self.root,text="CLEAR URL",width=25,bg="green",command=self.clear_url).place(x=280,y=70)
        ...
        self.linkBox = tk.Listbox(self.canvas,height=28,width=28)# 32 55
        ...
        self.searchEntry = tk.Entry(self.root,textvariable=self.search,font=("arial",14),width=12)

        self.searchEntry.place(x=250,y=110)

        tk.Button(self.root,text="SEARCH",bg="darkgreen",command=self.search_name).place(x=385,y=107)

        self.showAll = tk.Button(self.root,text="SHOW ALL LINKS",width=20,command=self.show_all)

        self.showAll.place(x=256,y=138)

        self.numLinks = tk.Label(self.root,text='{} LINKS'.format(len(self.link_list)),bg='black',fg='green',width=16,font=("arial",10))

        self.numLinks.place(x=256,y=114)#205

        tk.Button(self.root,text="IMPORT NEW LINK",bg="red",width=28,height=2,command=self.init_copy).place(x=363,y=250)#245

        tk.Button(self.root,text="ACCESS",bg="blue",width=28,height=2,command=self.init_task).place(x=363,y=300)#295

        tk.Button(self.root,text="DELETE",bg="yellow",width=28,height=2,command=self.remove_link).place(x=363,y=350)#365

        tk.Button(self.root,text="DELETE ALL",bg="pink",width=28,height=2,command=self.delete_listbox).place(x=363,y=400)#415
